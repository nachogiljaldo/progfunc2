...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    initial match {
      case head #:: tail =>
        head #:: from(tail #::: newNeighborsOnly(neighborsWithHistory(head._1, head._2), explored), explored + head._1)
   ...
   */
  lazy val pathsToGoal: Stream[(Block, List[Move])] = pathsFromStart filter { element =>
    done(element._1)
  }
